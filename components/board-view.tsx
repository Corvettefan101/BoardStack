...
export function BoardView({ board }: BoardViewProps) {
  ...
  const { createColumn, updateCounter } = useUserBoards()
  ...
  console.log("BoardView render - board:", board.id, "columns:", board.columns?.length, "updateCounter:", updateCounter)

  return (
    <div className="flex space-x-6 overflow-x-auto pb-6" onDragOver={handleDragOver}>
      {board.columns?.map((column) => (
        <ColumnComponent key={`${column.id}-${updateCounter}`} column={column} />
      ))}
